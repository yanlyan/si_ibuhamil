...
class m_site extends CI_Model {
    ...
    function get_navigation_user_by_parent($params) {
        ...
        WHERE a.portal_id = ? AND c.role_id = ? AND parent_id = ? AND active_st = '1' AND display_st = '1'
        GROUP BY a.nav_id
        ORDER BY nav_no ASC";
        $query = $this->db->query($sql, $params);
        if ($query->num_rows() > 0) {
    ...
    // get navigation by user and parent nav
    function get_navigation_user_by_parent_custom($params) {
        $sql = "SELECT * FROM com_menu a
        INNER JOIN com_role_menu b ON a.nav_id = b.nav_id
        INNER JOIN com_role_user c ON b.role_id = c.role_id
        WHERE a.portal_id = ? AND c.role_id IN
        (
            SELECT c.role_id FROM com_user a
        ...
        AND parent_id = ? AND active_st = '1' AND display_st = '1'
        GROUP BY a.nav_id
        ORDER BY nav_no ASC";
        $query = $this->db->query($sql, $params);
        if ($query->num_rows() > 0) {
    ...
    // get navigation by parent nav lang
    function get_navigation_by_parent_lang($params) {
        $sql = "SELECT a.*, lang_label FROM com_menu a
        LEFT JOIN com_menu_lang b ON a.nav_id = b.nav_id
        WHERE portal_id = ? AND parent_id = ? AND active_st = '1' AND display_st = '1' AND b.lang_id = ?
        ORDER BY nav_no ASC";
        $query = $this->db->query($sql, $params);
        // echo $this->db->last_query();
        if ($query->num_rows() > 0) {
    ...
    function get_user_authority_by_nav($params) {
        ...
        //         INNER JOIN com_role c ON b.role_id = c.role_id
        //         INNER JOIN com_role_user d ON c.role_id = d.role_id
        //         WHERE c.role_id = ? AND b.nav_id = ? AND active_st = '1' AND a.portal_id = ?";
        $sql = "SELECT b.* FROM com_menu a
        INNER JOIN com_role_menu b ON a.nav_id = b.nav_id
        INNER JOIN com_role c ON b.role_id = c.role_id
        INNER JOIN com_role_user d ON c.role_id = d.role_id
        ...
        AND b.nav_id = ? AND active_st = '1' AND a.portal_id = ?";
        $query = $this->db->query($sql, $params);
        // echo $this->db->last_query();
        if ($query->num_rows() > 0) {
            $result = $query->row_array();
